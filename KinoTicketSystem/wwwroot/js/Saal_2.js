...
function generateRowHTML(rowLetter, seats, color) {
    return `
        <div class="row" id="row-${rowLetter}">
            ${seats.map(seat => generateSeatHTML(seat, color)).join("")}
...
async function reserveSeatTemp(seatId, isTemporary = true) {
    try {
        ...
        let requestBody = JSON.stringify({ seatId, isTemporary, userId: userData.id });

        let response = await fetch("/api/seats/reserve", { method: "POST", headers: { "Content-Type": "application/json" }, body: requestBody });
        if (!response.ok) throw new Error(`Reservierung fehlgeschlagen: ${response.status} - ${await response.text()}`);
    } catch (error) {
...
async function redirectToSummary() {
    ...
    if (ticketCount === "0") {
        ...
        window.location.href = "../html/Saal_2_Summary.html";
    }
}
